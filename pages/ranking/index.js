...
export default function RankingPage({ schools = [], rankingType = '', city = '', level = null, isPrivate = null, error = null }) {
  ...
  if (error || !schools) {
    return (
      <>
        <Head>
          <title>Error | Encuentra Tu Escuela</title>
        </Head>
        <Header currentPage="ranking" />
        <div className="min-h-screen flex items-center justify-center">
          <p className="text-red-600 text-lg">❌ {error || 'Error loading ranking'}</p>
        </div>
        <Footer />
      </>
    );
              ...
              {schools.length > 0 ? (
                <div className="space-y-4">
                  {schools.map((school, index) => (
                    <div key={school._id || school.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition border border-gray-100 overflow-hidden">
                      <div className="p-6">
